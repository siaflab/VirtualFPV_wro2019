...
public class DummySender : MonoBehaviour {
    ...
    void ReadLogfile()
    {
        try
        {
            ...
            using (var sr = new System.IO.StreamReader(projectFolder + "/logdata/gps_data.txt"))
            {
    ...
    void ReadSimlationCSV()
    {
        try
        {
            ...
            using (var sr = new System.IO.StreamReader(projectFolder + "/simdata/flight_path.csv"))
            {
                while (!sr.EndOfStream)
                {
                    var line = sr.ReadLine();
    ...
    float GetSecond(string s)
    {
        ...
        float ret = float.Parse(Right(timeToken[0], 2)) * 3600f
    ...
    float GetDiffTime(int n)
    {
        ...
        string[] timeToken = data[3].Split(':');
        float time = float.Parse(Right(timeToken[0], 2)) * 3600
